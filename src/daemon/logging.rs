...
impl GlobalLogger {
    pub fn get() -> &'static GlobalLogger {
        GLOBAL_LOGGER.get().expect("Trying to get GLOBAL_LOGGER before it was setup")
    }
}


fn setup_syslog_drain(config: &Configuration) -> Option<slog::Fuse<slog_syslog::Streamer3164>> {
    config.logging.syslog.as_ref().map(|conf| {
        if !conf.enabled {
            return None;
        }
        let level = {
